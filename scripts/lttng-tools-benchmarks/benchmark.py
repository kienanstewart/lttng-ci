...
s3conf.S3_STORAGE_PATH = "{}results/benchmarks/lttng-tools".format(
    "{}/".format(s3conf.S3_BASE_DIR) if s3conf.S3_BASE_DIR else ""
)

default_branches = {
    "master": "da70f9608bc127824ecb6f602a14d9321d63c583",
    "stable-2.16": "9878ae1e8e09dc74e9c4f7bc48befe4cacea67e9",
            ...
            for metric in result["data"]:
                ...
                if version not in benchmarks_by_name_and_version[benchmark_name]:
                    ...
                    if metric in result["metrics"]:
                        unit = result["metrics"][metric].get("unit", "<unknown>")

                    if unit is None:
                        unit = "<unknown>"
                        logging.debug(
                            "Benchmark {} metric {} unit is '{}' at version {}".format(
                                benchmark_name, metric, unit, version
                            )
                        )
...
if __name__ == "__main__":
    ...
    logging.info("Storage path: {}".format(s3conf.S3_STORAGE_PATH))

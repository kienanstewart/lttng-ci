...
def _get_parser():
    parser = benchmark.get_parser(
        description="LTTng-UST benchmark launcher", default_branches=default_branches
    )
    ...
    benchmark.add_regression_check_parser(
        subparsers,
        func=lambda args: benchmark.cmd_regression_check(
            args, get_benchmark_state, get_benchmark_results, regression_check
        ),
    )
    return parser

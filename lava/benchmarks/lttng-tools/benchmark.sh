...
TAILLEUR_SRC_DIR="${SRC_DIR}/tailleur"
TOOLS_SRC_DIR="${SRC_DIR}/lttng-tools"
TOOLS_BENCHMARKS_SRC_DIR="${SRC_DIR}/lttng-tools-benchmarks"
...
git clone --quiet "${LTTNG_TOOLS_BENCHMARKS_REPO}" "${TOOLS_BENCHMARKS_SRC_DIR}"
if [[ "${LTTNG_TOOLS_BENCHMARKS_BRANCH}" =~ ^refs/ ]]; then
    git -C "${TOOLS_BENCHMARKS_SRC_DIR}" fetch --quiet origin "${LTTNG_TOOLS_BENCHMARKS_BRANCH}"
    git -C "${TOOLS_BENCHMARKS_SRC_DIR}" checkout FETCH_HEAD
else
    git -C "${TOOLS_BENCHMARKS_SRC_DIR}" checkout "${LTTNG_TOOLS_BENCHMARKS_BRANCH}"
fi

git clone --quiet "${TAILLEUR_REPO}" "${TAILLEUR_SRC_DIR}"
if [[ "${TAILLEUR_BRANCH}" =~ ^refs/ ]]; then
    git -C "${TAILLEUR_SRC_DIR}" fetch --quiet origin "${TAILLEUR_BRANCH}"
    git -C "${TAILLEUR_SRC_DIR}" checkout FETCH_HEAD
else
    git -C "${TAILLEUR_SRC_DIR}" checkout "${TAILLEUR_BRANCH}"
fi
...
function set_commits_from_tools_commit()
{
    local commit="${1:-${TOOLS_COMMIT}}"

    # The first heuristic is the nearest tag. This is _not_ very good on
    # the master branch, but otherwise it's probably "ok".
    tag="$(git -C "${TOOLS_SRC_DIR}" describe --abbrev=0 "${commit}")"
    vmajor_minor="$(echo "$tag" | cut -d '.' -f 1-2)"

    # In general, use the .0 release of the major.minor.
    ust_commit="${vmajor_minor}.0"
    urcu_commit="v0.15.0"
...
function build_modules()
{
    ...
    mkdir -p "${PREFIX}/usr/lib/$(uname -r)"
...
function build_tools_benchmarks()
{
    make -C "${TOOLS_BENCHMARKS_SRC_DIR}" clean \
                EXTRA_CFLAGS="${DEFAULT_CFLAGS}" \
                EXTRA_CXXFLAGS="${DEFAULT_CXXFLAGS}" \
                EXTRA_LDFLAGS="${DEFAULT_LDFLAGS}"
    make -C "${TOOLS_BENCHMARKS_SRC_DIR}" -j"$(nproc)" \
                EXTRA_CFLAGS="${DEFAULT_CFLAGS}" \
                EXTRA_CXXFLAGS="${DEFAULT_CXXFLAGS}" \
                EXTRA_LDFLAGS="${DEFAULT_LDFLAGS}"
}
...
export PYTHONPATH="${TAILLEUR_SRC_DIR}:${PREFIX}/lib/python${python_version}/site-packages"
...
while read -d ' ' -r commit; do
    ...
    TOOLS_COMMIT="${commit}"
    RESULTS_DIR="results/benchmarks/lttng-tools/${TOOLS_COMMIT}"
    ...
    set_commits_from_tools_commit "${TOOLS_COMMIT}"
    ...
    # Clean and rebuild lttng-tools-benchmarks
    if ! build_tools_benchmarks; then
    ...
    if ! (
            set -e
            cd "${TOOLS_BENCHMARKS_SRC_DIR}"
            rm -f metadata.yaml
            cat > metadata.yaml <<EOF
---
lttng-tools_commit: "${TOOLS_COMMIT}"
lttng-tools_commit_description: "$(git -C "${TOOLS_SRC_DIR}" describe)"
lttng-ust_commit: "${ust_commit}"
urcu_commit: "${urcu_commit}"
babeltrace_commit: "${babeltrace_commit}"
lttng-modules_commit: "${modules_commit}"
lttng-tools-benchmarks_commit: "$(git rev-parse HEAD)"
EOF
            poetry install > "${BENCHMARK_LOG}" 2>&1
            poetry run tailleur -o benchmarks.json \
                   -m metadata.yaml \
                   --benchmarks benchmarks.yaml >> "${BENCHMARK_LOG}" 2>&1
        ) ; then
        ...
        upload_artifact "${TOOLS_BENCHMARKS_SRC_DIR}/benchmarks.json" "${RESULTS_DIR}/benchmarks.json"
